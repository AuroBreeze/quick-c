...
- 项目级配置文件支持 (`.quick-c.json`)
  - 支持在项目根目录创建 `.quick-c.json` 文件覆盖全局配置
  - 配置优先级：项目配置 > 用户配置 > 默认配置
  - 自动向上查找配置文件，支持嵌套项目结构
  - 配置格式与 Lua 配置相同，使用 JSON 格式
- 新增项目配置使用指南文档 (`PROJECT_CONFIG_GUIDE.md`)

### 改进
- 配置系统增强：支持多级配置合并
- 文档更新：中英文 README 添加项目配置文件说明
- 错误处理：改进配置文件的解析和验证
...
### 迁移指南
- 无需迁移。如需使用项目配置文件，在项目根目录创建 `.quick-c.json`
